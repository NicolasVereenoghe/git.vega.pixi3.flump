...
class ApplicationMatchSize extends Application {
	...
	function updateSize() : Void {
		...
		if ( _scrMsk == null){
			_scrMsk	= new Graphics();
			
			_scrMsk.beginFill( 0x0000FF);
			_scrMsk.drawRect( -_EXT_WIDTH / 2, -_EXT_HEIGHT / 2, _EXT_WIDTH, _EXT_HEIGHT);
			_scrMsk.endFill();
			
			_container.addChildAt( _scrMsk, 0);
			
			_container.mask = _scrMsk;
		}
