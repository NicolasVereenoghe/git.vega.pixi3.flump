...
class LocalMgr {
	...
	public function instanciateTxtFromFlumpModel( pTxtId : String, pModelInstance : Movie, pVal : String = null) : Text {
		...
		lParams	= {
			...
			//"fontFamily": lDesc.fontId,	// pixiv4
			//"fontSize": lDesc.size,		// pixiv4
			"fill": lDesc.color,
			"align": lDesc.align,
			"padding": lDesc.size
		};
	...
	function alignTxt( pTxt : Text, pDesc : TxtDescFlump) : Void {
		...
		/*if ( pDesc.vAlign == TxtDescFlump.V_ALIGN_CENTER) pTxt.y = -( pTxt.height - 2 * ( pTxt.style.padding != null ? pTxt.style.padding : 0)) / 2;
		else if ( pDesc.vAlign == TxtDescFlump.V_ALIGN_BOT) pTxt.y = -( pTxt.height - 2 * ( pTxt.style.padding != null ? pTxt.style.padding : 0));*/
		
		if ( pDesc.vAlign == TxtDescFlump.V_ALIGN_CENTER) pTxt.y = ( pDesc.size - pTxt.height) / 2;
		else if ( pDesc.vAlign == TxtDescFlump.V_ALIGN_BOT) pTxt.y = pDesc.size - pTxt.height;
	}
